...
参考 AegisHook 将任务约束、执行事实与当前调用分开的设计，模型现在接收版本化 JSON：任务描述、目标和操作约束，本机工作目录、Worker 意图、当前轮输入，以及配对的近期工具执行结果；末尾保留完整工具名和 JSON 参数（包括交互 Shell 的会话 ID）。

- 任务约束每次审查从数据库重新读取。自动发现的资产不视为新增授权；约束登记来源也不等同于逐条人工批准。任务上下文读取失败或约束超过 16,000 字节时转人工，避免遗漏禁止项后自动放行。
- 意图和当前轮输入只提供背景，不能要求模型改变裁决。历史仅保留按调用 ID 配对的执行事实，排除 Agent 自述、未执行请求和平台拒绝反馈；失败结果可能包含部分副作用。
- 审查历史最多保留 6 对，每对参数预览和结果各 2,000 字节；描述、目标、意图和当前轮输入各最多 4,000 字节，截断有明确标记。有限窗口不能用作全任务累计计数；当前调用参数不截断。
- 所有模型裁决（包括自动允许）的详情均保存实际提交模型的 JSON 及指纹。页面优先展示该快照中的当前轮输入、任务背景和配对工具记录，并可展开完整 JSON；较大的原始会话审计片段单独展示，不重复保存在自动允许记录中。旧版仅保留指纹的记录明确提示无法还原原文，不用当前数据补造历史。
