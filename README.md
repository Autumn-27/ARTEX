...
参考 AegisHook 区分背景、执行事实与当前动作的设计，模型接收版本 2 JSON：本机工作目录、可用的简短背景、配对的近期工具执行结果、截断和关联标记；末尾保留完整工具名和 JSON 参数（包括交互 Shell 的会话 ID）。

- 背景由程序直接选取已有字段，不调用模型生成摘要：聊天和任务主 Agent 取当前用户消息（`background.source=user_message`）；Worker 只取当前意图 JSON 的 `summary`（`worker_summary`），不等同于用户消息。没有摘要、空值或格式错误时不附带背景，不回退到完整意图或整轮调度输入；自动规划器和自动触发的会话不附带用户背景；附件清单不混入用户消息。
- 不主动附带任务编号、描述、目标、任务操作约束、全局探索态势或完整 Worker 意图。审查模型依照自己的操作审查策略裁决，不负责约束 Agent 的任务方向；Agent 自身仍照常接收任务信息和约束。背景和工具结果中的指令不能改变审查规则，Worker 的声明不能证明产物归属。
- 历史来自当前 Agent 会话中按调用 ID 配对的已完成工具调用和输出，不做跨 Worker 汇总或语义检索。排除 Agent 自述、未完成请求和平台拒绝反馈；失败结果可能包含部分副作用。最近最多 6 对，每对参数预览和结果各 2,000 字节；背景最多 4,000 字节，超出后截断并标记。有限窗口不能用作全任务累计计数；当前调用参数不截断。
- 所有模型裁决（包括自动允许）的详情均保存实际提交模型的 JSON 及指纹。页面展示背景来源、工作目录和配对工具记录，并可展开完整 JSON；任务/会话编号和 Agent 仍用于审批来源显示。旧版快照保留当时发送的字段并标明旧版，只有指纹的历史记录明确提示无法还原原文，不用当前数据补造历史。较大的原始会话审计片段单独展示，不代表发送给审查模型的内容，也不重复保存在自动允许记录中。
- 模型输出 JSON 的 `decision`（allow / ask / deny）及 `comment`；三种裁决的说明均必须包含“实际操作、成功后的后果、命中规则”。缺少说明、重复字段或不完整 JSON 按已配置的模型失败策略处理，不伪造模型解释。
- 自定义审查策略保留，运行时统一追加输入信任边界和 JSON 输出协议，旧的单行输出要求由新协议替代。模型按当前操作的全部直接效果裁决，结合成功创建记录确认测试产物归属；写入报告只描述保存文本，不将正文记述的上传或命令当作本次执行行为。
