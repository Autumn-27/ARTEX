...
const TaskArchiveFormatVersion = 1
...
// Each value in Tables is a JSON array generated by PostgreSQL row_to_json, which
// preserves exact IDs, timestamps, arrays and JSON payloads for restoration.
...
// RecoverTaskArchiveJobs makes active operations retryable after an unclean
// shutdown. Idempotent restore/import semantics keep already-completed pieces safe.
func (d *DB) RecoverTaskArchiveJobs() error {
	_, err := d.Exec(`UPDATE task_archives SET
 state=CASE state WHEN 'archiving' THEN 'archive_queued'
                  WHEN 'restoring' THEN 'restore_queued'
                  WHEN 'deleting' THEN 'delete_queued' ELSE state END,
 phase='recovered', error=''
...
func (d *DB) SnapshotTaskArchive(taskID int64) (*TaskArchiveSnapshot, error) {
	...
	snapshot := &TaskArchiveSnapshot{
		...
		ExclusiveAssetIDs: exclusiveAssetIDs, Tables: tables, DataCounts: counts, AggregateStats: stats,
	}
