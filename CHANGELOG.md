...
- 支持**页面一键更新**：系统配置页新增「版本与更新」卡片，可直接从 GitHub Release 检查并安装新版本；顶栏版本号旁在有新版时亮出提示胶囊，点击直达。链路为「下载当前平台发布包 → 比对 Release 的 `SHA256SUMS` → 用 `-h` 冒烟测试新二进制 → 暂存为 `artex.new` → 进程以退出码 75 退出 → 守护脚本重新拉起并完成换装」，页面轮询 `/api/health` 等版本号变化后自动刷新。判据刻意是"版本变了"而非"能连上了"——换装过程中旧版本会短暂重新起来一次（那一次只负责把 `artex.new` 换上去然后立刻退出），只看连通性会误判成功。走完整 zip 而非裸二进制，是因为现有 Release 的 `SHA256SUMS` 本就只覆盖 zip，这样不必改 CI，也兼容已发布的历史版本。
- 新增守护启动脚本 `start.sh` / `start.bat` 作为正式启动入口（发布包与 Docker 镜像均已带上，`install.sh` 不变）。它只按退出码决定是否重新拉起：`0` = 正常停止、退出循环，`75` = 应用新版本、立刻重跑，其余 = 崩溃并按 1→2→4…最多 60 秒退避重启。刻意不在脚本里做校验与换装——那些逻辑在 sh 和 bat 上要写两套，而它们恰恰最不能出错：一旦换上跑不起来的二进制，守护进程会忠实地反复拉起它，用户只能上机器手工救。因此校验 / 冒烟 / 换装 / 回滚全部收在 Go 里（`selfupdate` 包），由 artex 自己在启动时完成，脚本保持傻瓜化。Docker 的 ENTRYPOINT 一并改为该脚本，它同时负责把 SIGTERM 转发给 artex——`docker stop` 只把信号发给 PID 1，不转发的话 artex 收不到信号、做不了优雅关闭，10 秒后被 SIGKILL 硬杀，正在跑的任务直接断在半路。
- 更新失败不会留下坏程序：校验或冒烟不通过就丢弃暂存件、继续跑当前版本；换装后的新版若连续 3 次启动失败则自动回滚到 `artex.old`（起不来的那个留作 `artex.failed` 供排查）。两道闸门互补——「根本无法 exec」（下载截断、架构选错、缺依赖）由换装前的冒烟挡住，「能执行但初始化时崩溃」（配置不兼容、端口被占）由启动计数兜住，少任何一道都有漏网。设置页另有「回滚到上一版本」可主动退回，但数据库结构不会回退，旧版本可能无法识别新版写入的数据。
- 后端对 GitHub 的查询结果缓存 30 分钟（失败结果 2 分钟，网络恢复后能很快自愈）：未认证的 GitHub API 是每 IP 每小时 60 次，而顶栏提示每次整页加载都会查一次，且这 60 次是**服务器 IP** 上所有用户共用的——不缓存的话多开几个标签页或刷几次页面就把配额耗光，之后顶栏提示不再出现、点「检查更新」也报错，真想更新时反而点不动。用户显式点「检查更新」时带 `force=1` 绕过缓存拿实时结果（人工触发，频率天然有限）。取数期间全程持锁，并发请求排队等同一次查询而不是各自去打 GitHub。
- 安全约束：更新只从 GitHub 域名下载并强制 HTTPS，每一跳重定向都校验域名；发布源写死、不做成配置项——更新源可配等于给任何能改配置的人一条远程代码执行通道，对渗透测试平台这个口子开不得。所有升级文件都挂在**可执行文件所在目录**而非 CWD：服务化运行时工作目录可能是 `/`，用 CWD 会让暂存件落到别处、换装彻底失效。开发构建（版本号为 `dev` 或 `git describe` 带后缀的不可比较版本）禁用一键更新，避免正式版覆盖掉本地正在调试的二进制。

#### 已知限制

- Docker 下只换程序、不换镜像：镜像里的 playwright / nmap / node 等工具链不会跟着升级，新版若依赖新工具会坏；且 `docker compose up -d` 重建容器后会退回镜像自带的版本（换装写的是容器可写层）。要连镜像一起升级仍请用 `docker compose pull artex && docker compose up -d artex`。界面与确认弹窗均已就此提示。
- 更新只替换二进制，不同步发布包里的 `skills/`，新版新增的内置 skill 不会自动生效（原先靠 `update.sh` 的 `git pull` 顺带解决；Docker 下 `./skills:/app/skills` 的绑定挂载也会盖掉镜像内置的那份）。
- 更新即重启，会中断正在运行的任务，目前不做拦截，仅在确认弹窗中提示。
