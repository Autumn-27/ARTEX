...
- `SideQuestionService` 直接调用该具体 Provider 一次，不创建 agent session，不接入工具执行器、主 transcript、活动流或任务图，也不经过任务模型切换链。保留工具定义是为兼容既有结构化工具上下文；新返回的工具调用没有执行路径。仅返回工具调用时给出无法执行的提示。
...
历史完整保存，按序号游标每页最多返回 20 条。模型请求仅回放最近 20 组成功问答；保守估算请求长度并预留输出预算，优先移除旧旁路问答，基础上下文仍超限时返回明确错误。
